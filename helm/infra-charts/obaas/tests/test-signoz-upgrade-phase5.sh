...
STAGE1="${CHART_DIR}/examples/values-signoz-0.133-stage1.yaml"
...
assert_contains "${CHART_DIR}/Chart.yaml" 'version: "0.133.0"'
assert_contains "${CHART_DIR}/Chart.lock" 'version: 0.133.0'
...
assert_contains "${stage2_render}" 'image: docker.io/signoz/signoz:v0.133.0'
...
run_resource() {
  env KUBECTL="${MOCK_KUBECTL}" MOCK_SCENARIO="$1" NAMESPACE=obaas RELEASE_NAME=obaas \
    VALIDATION_TIMEOUT=10m SIGNOZ_VERSION=v0.133.0 COLLECTOR_VERSION=v0.144.6 \
