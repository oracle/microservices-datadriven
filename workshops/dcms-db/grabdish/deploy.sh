...
# Parameters:
DB_LOCKDOWN=${1-no} # yes / no(default)
DB_OCID=${2-null}   # Required if $1 is yes
UI_LOCKDOWN=${2-no} # yes / no(default)
DB_ALIAS=$3         # TNS alias of the DB (required)

# Make sure this is executed and not sourced
if (return 0 2>/dev/null) ; then
  echo "ERROR: Usage './deploy.sh'"
  exit 1
fi
...
if test "$DB_LOCKDOWN" == 'yes'; then
  if test "$DB_OCID" == 'null'; then
    echo 'Error: Parameter $2 must be set to the DB_OCID'
