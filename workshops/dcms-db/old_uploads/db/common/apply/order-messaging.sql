...
as
  ...
  function dequeue_inventory_message(in_wait_option in binary_integer) return json_object_t;
end order_messaging;
/
show errors
...
as
  ...
  order_service_name    constant varchar2(100) := '$ORDER_SERVICE_NAME'; -- Subscriber Name
  ...
  begin
    ...
    dequeue_options.consumer_name := order_service_name;
    dequeue_options.navigation    := dbms_aq.first_message;  -- Required for TEQ

    dbms_aq.dequeue(
      queue_name         => inventory_queue_name,
