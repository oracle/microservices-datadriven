...
sed -e  "s|%EXPORTER_NAME%|banka|g" db-metrics-exporter-banka-deployment-${CURRENTTIME}.yaml > /tmp/db-metrics-exporter-banka-deployment-$CURRENTTIME.yaml
mv -- /tmp/db-metrics-exporter-banka-deployment-$CURRENTTIME.yaml db-metrics-exporter-banka-deployment-$CURRENTTIME.yaml
...
if [ -z "$1" ]; then
    kubectl apply -f $SCRIPT_DIR/db-metrics-exporter-banka-deployment-$CURRENTTIME.yaml -n msdataworkshop
else
    kubectl apply -f <(istioctl kube-inject -f $SCRIPT_DIR/db-metrics-exporter-banka-deployment-$CURRENTTIME.yaml) -n msdataworkshop
fi
#todo service could be dynamic as well
kubectl apply -f $SCRIPT_DIR/db-metrics-exporter-banka-service.yaml -n msdataworkshop
