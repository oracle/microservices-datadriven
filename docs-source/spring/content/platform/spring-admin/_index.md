...
1. Expose the Spring Admin dashboard using this command:

    ```shell
    kubectl -n admin-server port-forward svc/admin-server 8989:8989
    ```

2. Open the Spring Admin dashboard at URL <http://localhost:8989>
...
3. On the web page, navigate to the Applications tab:
    ...
    * If you deployed the [sample apps](../../sample-apps), find the "SLOW" entry and click on it to expand
    * Click on the instance of the service
    * You can see details about the service instance, metrics, configuration, and so on.
