...
Oracle Backend for Spring Boot and Microservices provides HashiCorp Vault as part of the platform.
...
- [HashiCorp Vault Grafana Dashboard](#hashicorp-vault-grafana-dashboard)
...
- [Key/Value Secrets Engine](https://developer.hashicorp.com/vault/docs/secrets/kv). The Key/Value secrets engine is a generic non-versioned Key/Value store used to store arbitrary secrets.
...
{{< hint type=[warning] icon=gdoc_check title=Warning >}}
Losing the token can result in loss of access to the Vault.
{{< /hint >}}
...
A preinstalled Grafana dashboard is provided. [More Information](../../observability/metrics/)
