...
echo create inventory-helidon-se OAM Component and ApplicationConfiguration
...
sed -i "s|%ORDER_PDB_NAME%|${ORDER_PDB_NAME}|g" inventory-helidon-se-comp-${CURRENTTIME}.yaml
...
if [ -z "$1" ]; then
    ...
    kubectl apply -f $SCRIPT_DIR/inventory-helidon-se-app.yaml
else
    kubectl apply -f <(istioctl kube-inject -f $SCRIPT_DIR/inventory-helidon-se-comp-$CURRENTTIME.yaml) -n msdataworkshop
fi
