resource "oci_core_vcn" "okell_vcn" {
  ...
  display_name   = "travelbookingsaga"
  dns_label    = "travelbookingsaga"
}
...
/*resource "oci_core_dhcp_options" "travelbookingsaga" {
    #Required
    compartment_id = var.ociCompartmentOcid
    options {
    ...
    options {
        type = "SearchDomain"
        search_domain_names = [ "travelbookingsaga.oraclevcn.com" ]
    }
