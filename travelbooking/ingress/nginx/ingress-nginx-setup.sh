...
while ! state_done NGINX_HELM_REPO; do
  if helm repo add ingress-nginx https://kubernetes.github.io/ingress-nginx 2>$TRAVELBOOKING_LOG/nginx_repo_err; then
    helm repo update
    state_set_done NGINX_HELM_REPO
  else
    echo "Add NGINX to Helm Repo failed.  Retrying..."
    cat $TRAVELBOOKING_LOG/nginx_repo_err
    sleep 10
  fi
done
...
while ! state_done NGINX_NAMESPACE; do
  if kubectl create -f $TRAVELBOOKING_HOME/ingress/nginx/ingress-nginx-namespace.yaml 2>$TRAVELBOOKING_LOG/nginx_ingress_ns_err; then
...
while ! state_done SSL_SECRET_INGRESS; do
  if kubectl create secret tls ssl-certificate-secret --key $TRAVELBOOKING_HOME/tls/tls.key --cert $TRAVELBOOKING_HOME/tls/tls.crt -n ingress-nginx; then
...
while ! state_done NGINX_INGRESS_SETUP; do
  if helm install ingress-nginx ingress-nginx/ingress-nginx --namespace ingress-nginx --values $TRAVELBOOKING_HOME/ingress/nginx/ingress-nginx-helm-values4oci.yaml 2>$TRAVELBOOKING_LOG/nginx_ingress_err; then
    state_set_done NGINX_INGRESS_SETUP
  else
    echo "Ingress Controller installation failed.  Retrying..."
    cat $TRAVELBOOKING_LOG/nginx_ingress_err
    sleep 10
  fi
done
