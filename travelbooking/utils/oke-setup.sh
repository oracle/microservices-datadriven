...
while ! state_done SSL; do
  mkdir -p $TRAVELBOOKING_HOME/tls
  openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout $TRAVELBOOKING_HOME/tls/tls.key -out $TRAVELBOOKING_HOME/tls/tls.crt -subj "/CN=travelbookingsaga/O=travelbookingsaga"
  state_set_done SSL
done
