...
while ! state_done TRAVELAGENCYDB_OCID; do
  echo "`date`: Waiting for TRAVELAGENCYDB_OCID"
  sleep 2
done
...
while ! state_done PARTICIPANTDB_OCID; do
  echo "`date`: Waiting for PARTICIPANTDB_OCID"
  sleep 2
done
...
while ! state_done CWALLET_SSO_AUTH_URL; do
  ACCESS_URI=`oci os preauth-request create --object-name 'cwallet.sso' --access-type 'ObjectRead' --bucket-name "$(state_get RUN_NAME)" --name 'travelbookingsaga' --time-expires $(date '+%Y-%m-%d' --date '+7 days') --query 'data."access-uri"' --raw-output`
...
while ! state_done TRAVELAGENCYDB_PASSWORD_SET; do
  echo "`date`: Waiting for TRAVELAGENCYDB_PASSWORD_SET"
  sleep 2
done
...
while ! state_done PARTICIPANTDB_PASSWORD_SET; do
  echo "`date`: Waiting for PARTICIPANTDB_PASSWORD_SET"
  sleep 2
done
...
create table participant (
  participantid varchar(16) PRIMARY KEY NOT NULL,
  participantlocation varchar(32),
  participantcount integer CONSTRAINT positive_participant CHECK (participantcount >= 0) );

insert into participant values ('sushi', '1468 WEBSTER ST,San Francisco,CA', 0);
insert into participant values ('pizza', '1469 WEBSTER ST,San Francisco,CA', 0);
insert into participant values ('burger', '1470 WEBSTER ST,San Francisco,CA', 0);
commit;
!
...
# Order DB Link
while ! state_done TRAVELAGENCYDB_LINK; do
  U=$ORDER_USER
  SVC=$TRAVELAGENCYDB_SVC
  TU=$INVENTORY_USER
...
/
!
  state_set_done TRAVELAGENCYDB_LINK
done

# Inventory DB Link
while ! state_done PARTICIPANTDB_LINK; do
  U=$INVENTORY_USER
  SVC=$PARTICIPANTDB_SVC
  TU=$ORDER_USER
...
/
!
  state_set_done PARTICIPANTDB_LINK
done
...
while ! state_done DOT_NET_PARTICIPANTDB_PROC; do
  U=$INVENTORY_USER
  SVC=$PARTICIPANTDB_SVC
  sqlplus /nolog <<!
WHENEVER SQLERROR EXIT 1
connect $U/"$DB_PASSWORD"@$SVC
@$TRAVELBOOKING_HOME/participant-dotnet/dequeueenqueue.sql
!
  state_set_done DOT_NET_PARTICIPANTDB_PROC
done
