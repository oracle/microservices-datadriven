...
# Check home is set
if test -z "$TRAVELBOOKING_HOME"; then
  echo "ERROR: This script requires TRAVELBOOKING_HOME to be set"
  exit
fi
...
while ! state_done RUN_TYPE; do
  if [[ "$HOME" =~ /home/ll[0-9]{1,5}_us ]]; then
    ...
    state_set RUN_NAME "travelbookingsaga$(state_get RESERVATION_ID)"
    state_set TRAVELAGENCYDB_NAME "ORDER$(state_get RESERVATION_ID)"
    state_set PARTICIPANTDB_NAME "INVENTORY$(state_get RESERVATION_ID)"
...
while ! state_done USER_OCID; do
  ...
  # Validate
  if test ""`oci iam user get --user-id "$USER_OCID" --query 'data."lifecycle-state"' --raw-output 2>$TRAVELBOOKING_LOG/user_ocid_err` == 'ACTIVE'; then
    state_set USER_OCID "$USER_OCID"
  else
    echo "That user OCID could not be validated"
    cat $TRAVELBOOKING_LOG/user_ocid_err
  fi
done
...
while ! state_done RUN_NAME; do
  ...
  if [[ "$DN" =~ ^[a-zA-Z][a-zA-Z0-9]{0,12}$ ]]; then
    ...
    state_set TRAVELAGENCYDB_NAME "$(state_get RUN_NAME)o"
    state_set PARTICIPANTDB_NAME "$(state_get RUN_NAME)i"
  else
...
if ! state_get JAVA_BUILDS; then
  if ps -ef | grep "$TRAVELBOOKING_HOME/utils/java-builds.sh" | grep -v grep; then
    echo "$TRAVELBOOKING_HOME/utils/java-builds.sh is already running"
  else
    echo "Executing java-builds.sh in the background"
    nohup $TRAVELBOOKING_HOME/utils/java-builds.sh &>> $TRAVELBOOKING_LOG/java-builds.log &
  fi
fi
...
if ! state_get PROVISIONING; then
  if ps -ef | grep "$TRAVELBOOKING_HOME/utils/terraform.sh" | grep -v grep; then
    echo "$TRAVELBOOKING_HOME/utils/terraform.sh is already running"
  else
    echo "Executing terraform.sh in the background"
    nohup $TRAVELBOOKING_HOME/utils/terraform.sh &>> $TRAVELBOOKING_LOG/terraform.log &
  fi
fi
...
while ! state_done DOCKER_REGISTRY; do
  if test $(state_get RUN_TYPE) -ne 3; then
    export OCI_CLI_PROFILE=$(state_get HOME_REGION)
    if ! TOKEN=`oci iam auth-token create  --user-id "$(state_get USER_OCID)" --description 'travelbookingsaga docker login' --query 'data.token' --raw-output 2>$TRAVELBOOKING_LOG/docker_registry_err`; then
      if grep UserCapacityExceeded $TRAVELBOOKING_LOG/docker_registry_err >/dev/null; then
        ...
        cat $TRAVELBOOKING_LOG/docker_registry_err
        exit
      fi
    fi
  else
...
if ! state_get OKE_SETUP; then
  if ps -ef | grep "$TRAVELBOOKING_HOME/utils/oke-setup.sh" | grep -v grep; then
    echo "$TRAVELBOOKING_HOME/utils/oke-setup.sh is already running"
  else
    echo "Executing oke-setup.sh in the background"
    nohup $TRAVELBOOKING_HOME/utils/oke-setup.sh &>>$TRAVELBOOKING_LOG/oke-setup.log &
  fi
fi
...
if ! state_get DB_SETUP; then
  if ps -ef | grep "$TRAVELBOOKING_HOME/utils/db-setup.sh" | grep -v grep; then
    echo "$TRAVELBOOKING_HOME/utils/db-setup.sh is already running"
  else
    echo "Executing db-setup.sh in the background"
    nohup $TRAVELBOOKING_HOME/utils/db-setup.sh &>>$TRAVELBOOKING_LOG/db-setup.log &
  fi
fi
...
if ! state_done DB_PASSWORD; then
  ...
  while true; do
    if test -z "$TEST_DB_PASSWORD"; then
      read -s -r -p "Enter the password to be used for the travelagency and participant databases: " PW
    else
      PW="$TEST_DB_PASSWORD"
    fi
...
if ! state_done PROVISIONING; then
  ...
  while ! state_done PROVISIONING; do
    LOGLINE=`tail -1 $TRAVELBOOKING_LOG/terraform.log`
...
while ! state_done PARTICIPANTDB_OCID; do
  PARTICIPANTDB_OCID=`oci db autonomous-database list --compartment-id "$(cat state/COMPARTMENT_OCID)" --query 'join('"' '"',data[?"display-name"=='"'PARTICIPANTDB'"'].id)' --raw-output`
  if [[ "$PARTICIPANTDB_OCID" =~ ocid1.autonomousdatabase* ]]; then
    state_set PARTICIPANTDB_OCID "$PARTICIPANTDB_OCID"
  else
    echo "ERROR: Incorrect Inventory DB OCID: $PARTICIPANTDB_OCID"
    exit
  fi
done
...
if ! state_done OKE_NAMESPACE; then
  ...
  while ! state_done OKE_NAMESPACE; do
    LOGLINE=`tail -1 $TRAVELBOOKING_LOG/state.log`
    echo -ne r"\033[2K\r${LOGLINE:0:120}"
    sleep 2
  done
  echo
...
# Set admin password in travelagency database
while ! state_done TRAVELAGENCYDB_PASSWORD_SET; do
  ...
  umask 22
  oci db autonomous-database update --autonomous-database-id "$(state_get TRAVELAGENCYDB_OCID)" --from-json "file://temp_params" >/dev/null
  rm temp_params
  state_set_done TRAVELAGENCYDB_PASSWORD_SET
done


# Set admin password in participant database
while ! state_done PARTICIPANTDB_PASSWORD_SET; do
  # get password from vault secret
  DB_PASSWORD=`kubectl get secret dbuser -n msdataworkshop --template={{.data.dbpassword}} | base64 --decode`
  umask 177
  echo '{"adminPassword": "'"$DB_PASSWORD"'"}' > temp_params
  ...
  oci db autonomous-database update --autonomous-database-id "$(state_get PARTICIPANTDB_OCID)" --from-json "file://temp_params" >/dev/null
  rm temp_params
  state_set_done PARTICIPANTDB_PASSWORD_SET
done
...
if ! state_get NGINX_INGRESS_SETUP_DONE; then
  if ps -ef | grep "$TRAVELBOOKING_HOME/ingress/nginx/ingress-nginx-setup.sh" | grep -v grep; then
    echo "$TRAVELBOOKING_HOME/ingress/nginx/ingress-nginx-setup.sh is already running"
  else
    echo "Executing ingress-nginx-setup.sh in the background"
    nohup $TRAVELBOOKING_HOME/ingress/nginx/ingress-nginx-setup.sh &>>$TRAVELBOOKING_LOG/ingress-nginx-setup.log &
  fi
fi
...
ps -ef | grep "$TRAVELBOOKING_HOME/utils" | grep -v grep
...
while ! state_done SETUP_VERIFIED; do
  ...
  if test "$NOT_DONE" -gt 0; then
    # echo "Log files are located in $TRAVELBOOKING_LOG"
    bgs=$bg_not_done
...
cd $TRAVELBOOKING_HOME
rm -f ~/travelbookingsaga-state.tgz
tar -czf ~/travelbookingsaga-state.tgz state