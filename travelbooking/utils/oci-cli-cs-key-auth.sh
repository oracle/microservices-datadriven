...
while ! state_done PUSH_OCI_CLI_KEY; do
  ...
  if ! oci iam user api-key upload --user-id $(state_get USER_OCID) --key-file ~/.oci/oci_api_key_public.pem &>$TRAVELBOOKING_LOG/err; then
    if grep KeyAlreadyExists $TRAVELBOOKING_LOG/err >/dev/null; then
