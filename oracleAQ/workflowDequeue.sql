...
BEGIN
    ...
    DBMS_AQ.DEQUEUE(queue_name => 'plsql_userQueue',    dequeue_options => user_dequeue_options, message_properties => message_properties, payload => user_message, msgid => message_handle);
    ...
    deli_dequeue_options.consumer_name := 'plsql_deliverySubscriber';
    DBMS_AQ.DEQUEUE(queue_name => 'plsql_deliveryQueue', dequeue_options => deli_dequeue_options, message_properties => message_properties, payload => delivery_message, msgid => message_handle);
    ...
    DBMS_AQ.DEQUEUE(queue_name => 'plsql_appQueue', dequeue_options => app_dequeue_options, message_properties => message_properties, payload => app_message, msgid => message_handle);
    DBMS_OUTPUT.PUT_LINE ('APP Message: ' || app_message.ORDERID || ' ... ' || app_message.USERNAME || ' ... ' || app_message.OTP);   

    -- get OTP from DB
    SELECT OTP INTO appOTP FROM USERDETAILS WHERE ORDERID=app_message.ORDERID;
